...
import FilterBar from '../components/MovieComponents/FilterBar';
...
const LoadingIndicator = () => (
  <div className="flex flex-col items-center justify-center py-20">
    ...
    <p className="mt-4 text-text-secondary-light dark:text-text-secondary-dark text-lg">Loading series...</p>
  </div>
);
...
const SeriesPage = () => {
  ...
  const fetchMoviesList = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api_series.php?${queryParams.toString()}`);
  ...
  useEffect(() => {
    ...
    if (selectedMovie && movieTitle) {
      document.title = `${movieTitle} - Series Details - IMDb2`;
    } else if (searchTerm) {
      document.title = `Search: ${searchTerm} - IMDb2`;
    } else {
      document.title = `Browse Series - IMDb2`;
    }
  ...
  if (searchTerm && !isLoading) {
    ...
    pageTitleContent = <>Browse Series{filterText} ({isLoading ? '...' : (totalMovieCount || 0)} total)</>;
  }
  ...
  return (
    <div className="container mx-auto px-2 sm:px-4 py-8 text-text-primary-light dark:text-text-primary-dark">
      ...
      <SearchBar onSearch={handleSearchSubmit} initialTerm={searchTerm} type='series'/>
      ...
      {selectedMovie && ( <MovieModal movie={selectedMovie} onClose={handleCloseMovieModal} type={"series"}/> )}
